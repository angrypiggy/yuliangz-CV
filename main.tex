    ...
    \resumeSubheading
      ...
      {Machine Learning Engineer}{Jan 2022 \textbf{--} Present}
        \resumeItemListStart
            ...
            \resumeItem{Contributed to Hover that enables \textbf{20M+ Apple Pencil users} to preview actions interactively before committing, improving precision, customization and workflow in daily tasks such as art creation and note-taking}
            \resumeItem{Perfected \textbf{adaptive filtering and bias correction} for the \textbf{Tilt/Azimuth Sensitivity} feature and developed key localization algorithms in C++, improving angle estimation accuracy by 28\% and smoothness by 8\%}
            \resumeItem{Built an \textbf{Apple Cloud Infrastructure} based evaluation pipeline in Python for machine learning models, achieving \textbf{6x time efficiency} and facilitating model iteration, leading to a 15\% accuracy enhancement }
            \vspace*{5pt}

            \item[]\small \hspace{-0.2in} \textit{Apple Pencil Pro Localization Algorithm Enhancement for iPad Pro 6th Gen} $|$ \emph{\href{https://www.youtube.com/watch?v=AK-ZMshDh14}{\color{blue}Showcase}}
            \resumeItem{Optimized the \textbf{pixel-precise pencil tip localization algorithms} in C++ based on mathematical models, addressing hardware noise challenges and \textbf{improving the prediction accuracy by 47\%} }
            \resumeItem{Established comprehensive unit test coverage in C\# to ensure the algorithm's robustness against 2000+ use cases}
            \vspace*{5pt}
